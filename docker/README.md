...
6. **Thiếu `website/game/template/`** (`perLoadTpls.json`, `dongtaitishi.txt`) — không có trong git, không bị gitignore, không có trong release `assets-v1`. Client vào được tới màn hình chọn máy chủ rồi đứng ở "Đang tải cấu hình giao diện… 0%". Phải lấy từ `/www/wwwroot/game/template` trên server cũ.
