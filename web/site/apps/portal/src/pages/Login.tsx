...
import { Button, Field, Msg } from "@op/site-ui";
  ...
  return (
    <AuthShell
      ...
      foot={
        <>
          Chưa có tài khoản?{" "}
          <a href={`/dang-ky${next !== "/tai-khoan" ? `?next=${encodeURIComponent(next)}` : ""}`}>Đăng ký</a>
    ...
    >
      <form
        onSubmit={(e) => {
          e.preventDefault();
          if (user.trim() && pass) login.mutate();
        }}
      >
        ...
        <Button type="submit" full size="lg" className="mt-1.5" disabled={login.isPending || !user.trim() || !pass}>
