...
- **Font chữ của game:** `assets/fonts/msyh.ttf` không phải Microsoft YaHei mà là **UTM Cafeta** (font Việt hẹp, 251 glyph); `play.php` gán nó đè lên tên `Arial`/`Helvetica`/`Times` và (từ 2026-09-07) khai báo thêm dưới tên thật `'UTM Cafeta'` + preload + hoãn nạp lib tới khi font xong — canvas trên iPhone từng rơi về Helvetica đậm (rộng gấp rưỡi) làm nhãn dài đè nhau. Nhãn nào cần font/cỡ riêng thì đặt `font`/`fontSize` trong `ui.bin` bằng tool (mẫu: `tools/va-nhan-canh-gioi.py`, màn Tu luyện), phát hành qua `phat-hanh-res.py ui/ui.bin`.
