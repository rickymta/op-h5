...
- **DB phải dump từ server cũ** (`docker/prepare-dumps.sh`): không JAR nào chứa schema `tcg/stat/web/cdks` hay Mongo. Dump vào `docker/initdb/{mysql,mongo/dump}` → import tự động lần đầu.
  Đo thực tế: với `tcg` **rỗng**, `console`, `world`, `meta`, `statistic` **vẫn khởi động** (log có `Started Tcg…Application`, chỉ kèm lỗi `Table … doesn't exist` khi truy vấn). Không chạy được là `game`, `group`, `cross` — chúng đọc `srv_game` / `srv_group_device` để biết mình là ai — và `login`, vì `/srv/game/list` và `/account/*` đều đọc bảng trong `tcg`. Hệ quả: client vào được tới màn hình đăng nhập nhưng không nối được WebSocket game. Xem `docker/dev-macos.sh`.
