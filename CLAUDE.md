...
- Bộ này **chưa từng được `up` thật** (máy chuẩn bị không có Docker); mới lint cú pháp compose và mô phỏng `zz-init.sh` bằng stub. Lần `up` đầu là lần test — bản giao việc cho phiên Claude trên máy macOS có Docker: [docs/mac-test-brief.md](docs/mac-test-brief.md) (chạy `docker/dev-macos.sh`, điểm kiểm tra, cách lần lỗi, quy tắc trước khi commit).
