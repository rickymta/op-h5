...
- **Font chữ của game:** `assets/fonts/msyh.ttf` không phải Microsoft YaHei mà là **UTM Cafeta** (font Việt hẹp, 251 glyph); `play.php` gán nó đè lên tên `Arial`/`Helvetica`/`Times` và (từ 2026-09-07) khai báo thêm dưới tên thật `'UTM Cafeta'` + preload + hoãn nạp lib tới khi font xong — canvas trên iPhone từng rơi về Helvetica đậm (rộng gấp rưỡi) làm nhãn dài đè nhau. Bundle nay đặt `Laya.Text.defaultFont='UTM Cafeta'` cho mọi nền tảng (`tools/va-tu-danh-ai.py`); ô hẹp mà tên dài thì hạ cỡ chữ theo bảng trong `tools/va-ui-nhan.py` (đo bằng canvas thật), phát hành qua `phat-hanh-res.py ui/ui.bin`.
- **Tự đánh ải tiếp** (`op-tu-danh.js`, 2026-09-07): bundle chỉ thêm một dòng gọi `window.opKetQuaTran` ở `afterOpen()` của màn thắng trận; shim bấm đúng nút "Khiêu chiến tầng tiếp" (đi qua kiểm tra thể lực/lượt của game) khi cờ `localStorage.opTuDanh === '1'` — nhận Excel/bundle mới thì chạy lại `va-tu-danh-ai.py`.
- **Nút "Cài đặt" tròn + hộp thoại** (`op-cai-dat.js`, 2026-09-07) thay cho nút "Trang chính" và nút "Tự đánh" DOM cũ: nút 38px kéo thả, thả ra tự hút về mép trái/phải (vị trí nhớ trong `localStorage.opCaiDatViTri` theo tỷ lệ chiều cao). Hộp thoại hai tab: *Cài đặt* (nhạc nền/hiệu ứng + âm lượng đặt thẳng vào `Laya.SoundManager` — chỉ áp khi người chơi đã chỉnh, khoá `opAmThanh`; công tắc `opTuDanh`/`opTuDiAi` mà hai shim kia đọc lại ở mỗi lần quyết định; Nạp Xu qua `openNapTien()`; Tài khoản/Trang chủ/Fanpage/Nhóm/Hỗ trợ từ `/api/game/meta`; Tải lại; Về trang chính qua `opRoiGame()`) và *Tin tức & thông báo* (`/meta/announce/one?gameId=` của meta server, `/api/game/news` của Adapter, link `/tin-tuc/<slug|id>` mở tab mới). Mọi chuỗi từ mạng ghi bằng `textContent`; hỏng API nào thì ẩn mục đó.
...
- **RAM:** tổng `-Xmx` gốc ≈ 8 GB, không vừa. Heap đã cắt (mặc định tổng 6016m trong `.env.example`; `group`/`cross` cần 1152m vì DB ghi 1128m, 640m OOM ngay — đo trên macOS; cả cụm thực dùng 2,3 GiB), `SerialGC`, `MaxMetaspaceSize=160m`, `Xss512k`; MySQL buffer pool 192M, Mongo cache 0.25 GB, MQ 256MB. Trần `mem_limit` cộng lại 9.69 GB nhưng peak không chồng nhau; **bắt buộc swap 4 GB**. Chỉ chạy 1 game server.
...
- Bộ này **đã `up` thật trên macOS 2026-09-05**: 19 container, 9 JVM, seed sạch nạp đủ 101 bảng (tcg 59 + stat 20 + web 21 + cdks 1), đăng ký → `/choi-game` → client tự đăng nhập bằng tài khoản game thật trên login server thật, RAM thực 2,3 GiB. Tài nguyên client đi qua bảng ánh xạ `libs/2af72-f100c-2af72.json` (tên logic → file băm trong `res/`), nên `template/`, `common/`, `atlas/` **không phải** thư mục trên đĩa — đừng đi tìm. Còn thiếu thật: 21 file băm trong `res/` + 1 file `sound/`, xem [docs/windows-assets-handoff.md](docs/windows-assets-handoff.md). 11 lỗi tìm được và cách vá: [docs/mac-test-brief.md](docs/mac-test-brief.md) mục 7.
...
- **Sửa một file trong `res/` thì KHÔNG ghi đè tại chỗ.** nginx phục vụ `/res/` và `/libs/` với
  ...
  `python tools/phat-hanh-res.py <tên logic> <file mới>`: tên băm mới + manifest
  `libs/2af72-f100c-2af72.json` trỏ sang (manifest trong git → đi theo image nginx; file res
  chép vào `ASSETS_DIR/res`). Manifest/loader đã `no-cache` và bust bằng `opManifestV`.
  Riêng `template/templates.bin` là bảng nhị phân theo khung `readInt32 count` +
  `readArrayBuffer(readInt16())` — chỉ sửa qua `tools/templates-bin.py` (có cổng kiểm mô
  phỏng parser client); xem [docs/vat-pham-lech-ban.md](docs/vat-pham-lech-ban.md). **Client ghi đè
  byte 10 của file nén thành 0x76 trước khi inflate** — nén bằng zlib thường là treo 40%; tool tự
  dựng luồng đúng, xem [docs/bang-client-theo-server.md](docs/bang-client-theo-server.md).
...
- **Giao diện `web/`** (tái cấu trúc 2026-09-06): hai nhóm tách hẳn, không import chéo. `web/admin/*` dùng **MUI** (`apps/platform` → `admin.<domain>`, `apps/gm` → `haitac.<domain>/admin-portal`); `web/site/*` dùng **Tailwind** (`apps/portal` → `<domain>`, `apps/market` → `<domain>/cho`, `apps/haitac` → `haitac.<domain>`). Năm bundle nhúng bằng `go:embed` vào ba binary; không còn cờ SPA và không còn template Go cũ. `cd web && npm ci && npm run build` trước khi build image; CI đã làm. Kế hoạch và trạng thái: [docs/plan-go-react.md](docs/plan-go-react.md), tham khảo giao diện: [docs/tham-khao-giao-dien-cong-game.md](docs/tham-khao-giao-dien-cong-game.md). Nhận diện game (tên, ảnh, màu, nhãn) nằm trong bảng `platform.games` (migration 0010), sửa ở trang quản trị → Game, **không** gắn cứng trong mã.
- **Hai cửa quản trị** (từ 2026-09-06): `https://admin.<domain>` và `https://haitac.<domain>/admin-portal` (302 sang `admin.<domain>/gm`) đều dẫn vào dịch vụ `admin`; nó vẫn bind `127.0.0.1:8100`, nginx là đường vào duy nhất (`docker/nginx/admin_access.conf` quyết định ai vào được). Bật bằng `ADMIN_PUBLIC=1`. **GM tool PHP `gmhanglong/` đã rút về loopback** — nó nối chuỗi SQL thẳng từ `$_GET`/`$_POST` mà lại phát vật phẩm; phần CDK chưa có bản Go nên vào bằng `ssh -L 8080:127.0.0.1:80`.
