...
- **Giao diện React `web/`** (từ 2026-09-05): ba app `apps/ops` → `admin`, `apps/portal` → `id` (trang chính + tài khoản), `apps/game` → `adapter` (một bundle cho mọi game), nhúng bằng `go:embed` từ `platform/cmd/<svc>/dist`, bật bằng `ADMIN_SPA`/`ID_SPA`/`ADAPTER_SPA=1` (trang Go cũ lui về `/cu/`). `cd web && npm ci && npm run build` trước khi build image; CI đã làm. Kế hoạch và trạng thái: [docs/plan-go-react.md](docs/plan-go-react.md), tham khảo giao diện: [docs/tham-khao-giao-dien-cong-game.md](docs/tham-khao-giao-dien-cong-game.md). Nhận diện game (tên, ảnh, màu, nhãn) nằm trong bảng `platform.games` (migration 0010), sửa ở trang quản trị → Game, **không** gắn cứng trong mã.
