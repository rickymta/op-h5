(function (exports, Laya) {
    ...
    class Templet extends AnimationTemplet {
        ...
        _parseTexturePath() {
            ...
            Laya.ILaya.loader.load(this._loadList, Laya.Handler.create(this, this._textureComplete, [this._loadList]));
        }
        _textureComplete(...arg) {
            var tTexture;
            var tTextureName;
            for (var i = 0, n = this._loadList.length; i < n; i++) {
                tTextureName = this._loadList[i];
                tTexture = Laya.ILaya.Loader.getRes(tTextureName);
                if(tTexture){
                    this._textureDic[tTextureName] = tTexture;
                }else{
                    if (Laya.Browser.window.ydwxReportError) {
                        Laya.Browser.window.ydwxReportError("layaError", "_textureComplete，加载完成的位图不存在：" + tTextureName + "，此时Templet是否被destroy：" + this._isDestroyed + "，加载是否全部成功：" + arg[1]);
                    }
                }
            }
            this._parsePublicExtData();
        ...
        destroy() {
            ...
            for (tTexture in this._textureDic) {
                if (tTexture) {
                    if(this._textureDic[tTexture]){
                        this._textureDic[tTexture].destroy();
                    }else{
                        if (Laya.Browser.window.ydwxReportError) {
                            Laya.Browser.window.ydwxReportError("layaError", "templet-destroy，试图释放不存在的位图：" + tTexture);
                        }
                    }
                }
            }
