...
if($xu <= 0 or $xu < $truxu){
	...
	$pdo->prepare("UPDATE `user` SET `xu` = `xu` - ? WHERE `username` = ?")->execute(array($truxu, $name));
	$pdo->prepare("INSERT INTO `log_xu` (`username`, `xu`) VALUES (?, ?)")->execute(array($username, $truxu));
	exit('true');
}
?>